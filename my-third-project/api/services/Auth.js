...
class AuthService {
  static async signIn({ userName, password, fingerprint }) {
    ...
    const payload = { role: userData.role, id: userData.id, userName, registrationDate: userData.registration_date };
  ...
  static async verifyRefreshToken(token) {
    if (!process.env.REFRESH_TOKEN_SECRET) {
      throw new Error("Секретный ключ не определен");
    }
    try {
      ...
      console.error("Ошибка при верификации refresh токена:", error.message);
